...
export class MenuItem extends ViewBase {
  ...
  @native({
    setNative(view: MenuItem, _key, value) {
      if (view.nativeView) {
        view.nativeView.state = value === 'on' ? NSControlStateValueOn : value === 'mixed' ? NSControlStateValueMixed : NSControlStateValueOff;
      }
    },
  })
  ...
  @native({
    setNative(view: MenuItem, _key, value) {
      if (view.nativeView) {
        const image = NSImage.imageWithSystemSymbolNameAccessibilityDescription(value, null);
