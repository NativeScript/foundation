...
import type { ToolbarItem } from './toolbar-item.js';
import type { ToolbarToggleSidebar } from './toolbar-toggle-sidebar.js';
import type { ToolbarSidebarTrackingSeparator } from './toolbar-sidebar-tracking-separator.js';
...
class ToolbarDelegate extends NSObject implements NSToolbarDelegate {
  ...
  toolbarItemForItemIdentifierWillBeInsertedIntoToolbar(_toolbar: NSToolbar, itemIdentifier: string, _flag: boolean): NSToolbarItem {
...
export class Toolbar extends ViewBase {
  ...
  public addNativeChild(child: ToolbarItem | ToolbarToggleSidebar | ToolbarSidebarTrackingSeparator): void {
    if (child.nodeName === 'TOOLBAR-ITEM' || child.nodeName === 'TOOLBAR-TOGGLE-SIDEBAR' || child.nodeName === 'TOOLBAR-SIDEBAR-TRACKING-SEPARATOR' || child.nodeName === 'TOOLBAR-FLEXIBLE-SPACE' || child.nodeName === 'TOOLBAR-GROUP') {
      this.toolbarDelegate!.items.push(child.nativeView!);
      this.reloadToolbar();
    } else {
      throw new Error(`Invalid child type added to toolbar: ${child.nodeName}`);
    }
  }
  ...
  public removeNativeChild(child: any): void {
    if (child.nodeName === 'TOOLBAR-ITEM' || child.nodeName === 'TOOLBAR-TOGGLE-SIDEBAR' || child.nodeName === 'TOOLBAR-SIDEBAR-TRACKING-SEPARATOR' || child.nodeName === 'TOOLBAR-FLEXIBLE-SPACE' || child.nodeName === 'TOOLBAR-GROUP') {
      ...
      throw new Error(`Invalid child type removed from toolbar: ${child.nodeName}`);
    }
  }
}
