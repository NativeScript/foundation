...
class NativeToolbarGroupTarget extends NSObject {
  ...
  public static initWithOwner(owner: WeakRef<ToolbarGroup>): NativeToolbarGroupTarget {
...
export class ToolbarGroup extends ToolbarItem {
  ...
  public override initNativeView(): NSToolbarItemGroup | undefined {
    ...
    this.nativeView = NSToolbarItemGroup.alloc().initWithItemIdentifier(crypto.randomUUID());
  ...
  public addNativeChild(child: ViewBase): void {
    if (child.nodeName === 'TOOLBAR-ITEM') {
      ...
      throw new Error(`Invalid child type added to toolbar-item: ${child.nodeName}`);
    }
  }
  ...
  public removeNativeChild(child: any): void {
    if (child.nodeName === 'TOOLBAR-ITEM') {
      ...
      throw new Error(`Invalid child type removed from toolbar-item: ${child.nodeName}`);
    }
  }
  ...
  @native({
    setNative: (view: ToolbarGroup, _key, value) => {
      if (view.nativeView) {
        switch (value) {
          case 'selectOne':
            view.nativeView.selectionMode = NSToolbarItemGroupSelectionMode.SelectOne;
            break;
          case 'selectAny':
            view.nativeView.selectionMode = NSToolbarItemGroupSelectionMode.SelectAny;
            break;
          case 'momentary':
            view.nativeView.selectionMode = NSToolbarItemGroupSelectionMode.Momentary;
            break;
          default:
  ...
  @native({
    setNative: (view: ToolbarGroup, _key, value) => {
      if (view.nativeView) {
        view.nativeView = NSToolbarItemGroup.groupWithItemIdentifierTitlesSelectionModeLabelsTargetAction(crypto.randomUUID(), value, view.nativeView.selectionMode, value, view.target, 'selected');

        view.nativeView.controlRepresentation = NSToolbarItemGroupControlRepresentation.Automatic;
      }
    },
  })
