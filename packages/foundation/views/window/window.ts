...
import { MainWindowController, NativeWindow } from './native-window.js';
...
export class Window extends ViewBase {
  ...
  public open() {
    if (NativeScriptApplication.window === this) {
      ...
      if (window && window?.windowController && this.nativeView) {
        const frameCentered = {
          x: window.frame.origin.x + window.frame.size.width / 2 - this.nativeView.frame.size.width / 2,
          y: window.frame.origin.y + window.frame.size.height / 2 - this.nativeView.frame.size.height / 2,
        };
  ...
  public openAsModal(relativeToWindow?: boolean) {
    if (relativeToWindow) {
      const window = (this.parentNode as HTMLViewElement).nativeView?.window;
      if (window) {
        this._modalCode = NSApp.runModalForWindowRelativeToWindow(this.nativeView!, window);
      }
    } else {
