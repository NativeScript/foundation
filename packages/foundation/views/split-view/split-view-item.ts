import { FlexStyle } from '../../layout/index.js';
import type { StylePropertyConfig } from '../../style/index.js';
import { parsePercent } from '../../style/utils/parse.js';
...
export class SplitViewItem extends ViewBase {
  ...
  public override initNativeView() {
    ...
    this.splitViewItem = NSSplitViewItem.splitViewItemWithViewController(this.viewController);
  ...
  public adjustSize() {
    if (this.viewController) {
      const { width, height } = this.viewController.view?.superview?.frame.size || { width: 0, height: 0 };
  ...
  @overrides('width')
  setWidth(key: string, value: FlexStyle['width'], config: StylePropertyConfig) {
    const width = this.getValue('width', value);
    config.setNative(this.style, key, width, config);
  }

  @overrides('height')
  setHeight(key: string, value: FlexStyle['height'], config: StylePropertyConfig) {
    const height = this.getValue('height', value);
    config.setNative(this.style, key, height, config);
  }

  @overrides('minWidth')
  setMinWidth(_key: string, value: FlexStyle['minWidth'], _config: StylePropertyConfig) {
  ...
  @overrides('maxWidth')
  setMaxWidth(_key: string, value: FlexStyle['maxWidth'], _config: StylePropertyConfig) {
    // config.setNative(this.style, key, value, config);
    // if (this.nativeView) {
  ...
  @overrides('minHeight')
  setMinHeight(_key: string, value: FlexStyle['minHeight'], _config: StylePropertyConfig) {
    // config.setNative(this.style, key, value, config);
    if (this.nativeView) {
      this.nativeView.heightAnchor.constraintGreaterThanOrEqualToConstant(this.getValue('height', value)).isActive = true;
    }
  }

  @overrides('maxHeight')
  setMaxHeight(_key: string, value: FlexStyle['maxHeight'], _config: StylePropertyConfig) {
    // config.setNative(this.style, key, value, config);
    if (this.nativeView) {
      this.nativeView.heightAnchor.constraintLessThanOrEqualToConstant(this.getValue('height', value)).isActive = true;
      this.adjustSize();
    }
  }
  ...
  getParentDimension(type: 'width' | 'height') {
    return (this.parentNode as HTMLViewElement).nativeView?.frame.size[type] || NativeScriptApplication.window.nativeView?.frame.size[type] || 0;
  }
}
