...
export class DatePicker extends View {
  ...
  public override initNativeView(): NSDatePicker | undefined {
    ...
    this.nativeView.dateValue = NSDate.dateWithTimeIntervalSince1970(Date.now() / 1000);
    const target = NativeTarget.initWithAction(() => {
      if (this.nativeView) {
        this.dispatchEvent(new DatePickerChangeEvent(new Date(this.nativeView.dateValue.timeIntervalSince1970 * 1000), this.nativeView.timeInterval * 1000));
      }
    });
  ...
  @native({
    setNative(view: DatePicker, _key, value) {
      if (view.nativeView) {
        view.nativeView.dateValue = NSDate.dateWithTimeIntervalSince1970(value.getTime() / 1000);
      }
    },
  })
  declare date: Date;
  ...
  @native({
    setNative(view: DatePicker, _key, value) {
      if (view.nativeView) {
        view.nativeView.minDate = NSDate.dateWithTimeIntervalSince1970(value.getTime() / 1000);
      }
    },
  })
  ...
  @native({
    setNative(view: DatePicker, _key, value) {
      if (view.nativeView) {
        view.nativeView.maxDate = NSDate.dateWithTimeIntervalSince1970(value.getTime() / 1000);
      }
    },
  })
  ...
  @native({
    setNative(view: DatePicker, _key, value) {
      if (view.nativeView) {
        view.nativeView.datePickerStyle = value === 'clockAndCalendar' ? NSDatePickerStyle.ClockAndCalendar : value === 'textField' ? NSDatePickerStyle.TextField : NSDatePickerStyle.TextFieldAndStepper;
      }
    },
  })
  declare datePickerStyle: 'textFieldAndStepper' | 'clockAndCalendar' | 'textField';
