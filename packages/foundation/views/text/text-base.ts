...
export class TextBase extends ViewBase {
  ...
  override onMeasureFunction(width: number, widthMode: MeasureMode, height: number, heightMode: MeasureMode) {
    ...
    return {
      width: this.measure(constrainedWidth, Math.ceil(size.width), widthMode),
      height: this.measure(constrainedHeight, Math.ceil(size.height), heightMode),
    };
  }
