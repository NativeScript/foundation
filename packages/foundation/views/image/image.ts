...
export class Image extends ViewBase {
  ...
  @native({
    setNative(view: Image, _key, value) {
      const img = NSImage.imageWithSystemSymbolNameAccessibilityDescription(value, null);
  ...
  @native({
    setNative(view: Image, _key, value: ImageStretch) {
      if (view.nativeView) {
        switch (value) {
          case 'aspectFit':
            view.nativeView.imageScaling = NSImageScaling.ImageScaleProportionallyUpOrDown;
            break;
          case 'fill':
            view.nativeView.imageScaling = NSImageScaling.ImageScaleAxesIndependently;
            break;
          case 'fit':
