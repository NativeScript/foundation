...
export function openFileDialog(options: FileDialogOptions) {
  return new Promise<Array<string | null>>((resolve, reject) => {
    ...
    if (options?.directoryUrl) {
      ...
      fileDialog.directoryURL = NSURL.fileURLWithPath(NSSearchPathForDirectoriesInDomains(NSSearchPathDirectory.Desktop, NSSearchPathDomainMask.UserDomain, true).firstObject);
    }
...
export function saveFileDialog(options: SaveFileDialogOptions) {
  return new Promise<string | undefined>((resolve, reject) => {
    ...
    if (options?.directoryUrl) {
      ...
      saveDialog.directoryURL = NSURL.fileURLWithPath(NSSearchPathForDirectoriesInDomains(NSSearchPathDirectory.Desktop, NSSearchPathDomainMask.UserDomain, true).firstObject);
    }
