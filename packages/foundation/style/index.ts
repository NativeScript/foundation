...
export type StylePropertyConfig<T = any> = {
  setNative: (style: Style, key: string, value: T, config: StylePropertyConfig<T>) => void;
  ...
  return function (_target: Style, property: string) {
    ...
    Object.defineProperty(_target, property, {
      ...
      set(value: T) {
        ...
        const hasChanged = !config.hasChanged ? oldValue !== value : config.hasChanged?.(oldValue, value);
...
export class Style extends Map {
  ...
  override set<Key extends keyof CombinedStyle>(key: Key, value: CombinedStyle[Key]): this {
    super.set(key, value);
    return this;
  }

  override get<Key extends keyof CombinedStyle>(key: Key): CombinedStyle[Key] | undefined {
